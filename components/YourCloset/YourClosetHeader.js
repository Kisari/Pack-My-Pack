...
const YourClosetHeader = ({ item_data_list }) => {
	...
	return (
		<Header
			centerComponent={{ text: "Your closet", style: { color: "#fff", fontSize: 28 } }}
			rightComponent={
				<SafeAreaProvider>
					<Button
						title="Connect"
						...
						buttonStyle={{ backgroundColor: "blue", borderRadius: 20, width: 150, height: 50 }}
					...
					<BottomSheet modalProps={{}} isVisible={isVisible} containerStyle={{ backgroundColor: "white" }}>
						{item_data_list.map((item_data, item_index) => (
							<ListItem key={item_index} containerStyle={{ backgroundColor: "white " }}>
								<Image style={styles.image} source={require("../../assets/Images/favicon.png")} />
								<ListItem.Content>
									<ListItem.Title style={{ color: "black" }}>{item_data.name}</ListItem.Title>
								</ListItem.Content>
							</ListItem>
						))}
